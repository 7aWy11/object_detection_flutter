...
void main() {
  ...
  final forceEnableTFLite =
      Platform.environment['ENABLE_TFLITE_TESTS'] == 'true';
  ...
  group('Classifier Tests', () {
    ...
    test('Load model and labels', () async {
      if (skipTFLiteTests) {
        markTestSkipped(
            'TensorFlow Lite not supported on Windows/Web. Set ENABLE_TFLITE_TESTS=true to force run.');
        return;
      }
      ...
      expect(classifier.labels!.isNotEmpty, true);
    },
        skip: skipTFLiteTests
            ? 'TensorFlow Lite not supported on Windows/Web. Set ENABLE_TFLITE_TESTS=true to force run.'
            : null);
    ...
    test('Predict with invalid image throws error', () async {
      if (skipTFLiteTests) {
        markTestSkipped(
            'TensorFlow Lite not supported on Windows/Web. Set ENABLE_TFLITE_TESTS=true to force run.');
        return;
      ...
      expect(
        () => classifier.predict(imageLib.Image(width: 0, height: 0)),
        throwsA(isA<Exception>()),
      );
    },
        skip: skipTFLiteTests
    ...
    test('Run object detection with invalid buffer throws error', () async {
      if (skipTFLiteTests) {
        markTestSkipped(
            'TensorFlow Lite not supported on Windows/Web. Set ENABLE_TFLITE_TESTS=true to force run.');
        return;
      ...
      final image = imageLib.Image(width: 300, height: 300);
      expect(
        () => classifier.runObjectDetection(Uint8List(0), image),
        throwsA(isA<Exception>()),
      );
    },
        skip: skipTFLiteTests
    ...
    test('Run classification with invalid buffer throws error', () async {
      if (skipTFLiteTests) {
        markTestSkipped(
            'TensorFlow Lite not supported on Windows/Web. Set ENABLE_TFLITE_TESTS=true to force run.');
        return;
      ...
      final image = imageLib.Image(width: 300, height: 300);
      expect(
        () => classifier.runClassification(Uint8List(0), image),
        throwsA(isA<Exception>()),
      );
    },
        skip: skipTFLiteTests
    ...
    test('Run YOLOv5 detection with invalid buffer throws error', () async {
      if (skipTFLiteTests) {
        markTestSkipped(
            'TensorFlow Lite not supported on Windows/Web. Set ENABLE_TFLITE_TESTS=true to force run.');
        return;
      ...
      final image = imageLib.Image(width: 300, height: 300);
      expect(
        () => classifier.runYolov5Detection(Uint8List(0), image),
        throwsA(isA<Exception>()),
      );
    },
        skip: skipTFLiteTests
    ...
    test('Image preprocessing with invalid image throws error', () {
      final classifier = Classifier();
      expect(
        () => classifier.preprocessImage(imageLib.Image(width: 0, height: 0)),
        throwsA(isA<Exception>()),
      );
    });
  });
